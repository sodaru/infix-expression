...
describe("Test Prefix Expression's evaluate for or operation", () => {
  ...
  test("with negative right", () => {
    expect(evaluate({ "||": [false, -3] })).toEqual(true);
  });
  ...
  test("with string", () => {
    expect(evaluate({ "||": ["shhhh", false] })).toEqual(true);
  });
  ...
  test("with object", () => {
    expect(evaluate({ "||": [false, { a: "x" }] })).toEqual(true);
  });

  test("with nested operation", () => {
    expect(evaluate({ "||": [false, { "||": [false, false] }] })).toEqual(
      false
    );
  });
