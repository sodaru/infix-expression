...
describe("Test Prefix Expression's evaluate for greaterThanOrEqual operation", () => {
  ...
  test("with two operands", () => {
    expect(evaluate({ ">=": [11, 3] })).toEqual(true);
  });
  ...
  test("with number string", () => {
    expect(evaluate({ ">=": [10, "3.5"] })).toEqual(true);
  });

  test("with boolean : true for right", () => {
    expect(evaluate({ ">=": [2, true] })).toEqual(true);
  });
  ...
  test("with null", () => {
    expect(evaluate({ ">=": [10, null] })).toEqual(true);
  });
  ...
  test("with nested operation", () => {
    expect(evaluate({ ">=": [11, { ">=": [5, 3] }] })).toEqual(true);
  });
